 ...
 * @date 9/23/21
 * @brief Set up for the letimer and its various properties
 *
 */
...
/***************************************************************************//**
 * @brief
 *   Driver to open an set an LETIMER peripheral in PWM mode, updated to include set up for Interrupt Handling
 *
 * @details
...
void letimer_pwm_open(LETIMER_TypeDef *letimer, APP_LETIMER_PWM_TypeDef *app_letimer_struct){
  ...
  // ZZZ are values that you must specify for this PWM specific driver from the online HAL documentation
	...
	letimer_pwm_values.enable = app_letimer_struct->enable; //app_letimer_pwm_struct
   ...
   */
    ...
    letimer->REP0 = REP_MODE_NUM; //could be 1
    letimer->REP1 = REP_MODE_NUM;
 ...
 *   directly interface to the LETIMER peripheral to turn-on or off its counter. Updated include sleep block mode interactions
 *
 * @note
 *   This function should only be called to enable/turn-on the LETIMER once the
 *   LETIMER peripheral has been completely configured via its open driver. Updated to update the sleep mode based on a check if the LETIMER is running and whether enable is true or false.
 *
 * @param[in] letimer
 *   Pointer to the base peripheral address of the LETIMER peripheral being opened
 *
...
void letimer_start(LETIMER_TypeDef *letimer, bool enable){
  if((letimer->STATUS & LETIMER_STATUS_RUNNING) && enable == false)
    {
      sleep_unblock_mode(LETIMER_EM);
      while(letimer->SYNCBUSY);
    }
  if(!(letimer->STATUS & LETIMER_STATUS_RUNNING) && enable == true)
    {
      sleep_block_mode(LETIMER_EM);
      while(letimer->SYNCBUSY);
    }
...
/***************************************************************************//**
 * @brief Sets a CallBack (event) to be serviced for each interrupt.
 *
 *
 * @details
 * Checks the interrupt flag and IEN bit of the LETIMER0. Based on the bit values, a callback event will
 * be scheduled.
 *
 * @note
 *   Called whenever callback events are to be scheduled.
